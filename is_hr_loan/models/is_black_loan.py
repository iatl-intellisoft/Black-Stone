...
class HrLoan(models.Model):
    ...
    def loan_validate(self):
        for rec in self:
            finance_approval = self.env['finance.approval']
            record = finance_approval.create({
                'requester': rec.requester,
                'fa_date': fields.Date.today(),
                'request_amount': rec.loan_amount_3,
                'amount': rec.loan_amount_3,
                'reason': self.name,
                'custody': 'False',
                'state': 'fm_app'
    ...
    def compute_total_deductions(self):
        for rec in self:
            if rec.employee_id:
                ...
                rec.total_deductions = si + sum_short_loans + sum_long_loans
