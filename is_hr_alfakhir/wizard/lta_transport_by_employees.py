...
class LtaTransportEmployees(models.TransientModel):
    ...
    employee_ids = fields.Many2many('hr.employee', 'lta_transport_group_rel', 'lta_transport_id', 'employee_id', 'Employees')
    ...
    def compute_grant_sheet(self):
        for rec in self:
            ...
            if active_id:
                [run_data] = rec.env['lta.transport'].browse(active_id).read(['name'])
            name = run_data.get('name')
            date = run_data.get('date')
            if not data['employee_ids']:
                raise UserError(_("You must select employee(s) to generate allowance(s)."))
