...
class FinanceApproval(models.Model):
    ...
    loan_payment_date = fields.Date('Loan Payment Date ')
    ...
    def get_loan_paid(self):
        ...
        for rec in con_ids:
            if rec.contract_end:
                ...
                alert_date = datetime.strptime(str(rec.loan_payment_date), '%Y-%m-%d')
                ...
                if days == 0:
                    ...
                    for mg in self.env.cr.fetchall():
                        vals = {
                            ...
                            'res_id': rec.id,
                            'res_model_id': rec.env['ir.model'].search([('model', 'like', 'finance.approval')],
                                                                       limit=1).id,
                            'user_id': mg[0] or 1,
                            'summary': 'payment of loans is today ' + rec.loan_payment_date,
                        }
                        print(vals)
                    # add lines
