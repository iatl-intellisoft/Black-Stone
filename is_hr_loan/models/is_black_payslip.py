...
# from is_hr_alfakhir.models.is_hr_contract import create
...
class HrLoan(models.Model):
    ...
    loan_amount_2 = fields.Float(string="Old Loans Amount", compute="all_old_loans",store=True)
    loan_amount_3 = fields.Float(string="Final Loan Amount", compute="compute_final_loan")
    ...
    state = fields.Selection(
        [('draft', 'To Submit'), ('approve', 'Admin Approved'),
         ('confirm', 'GM Approved'), ('admin_approve', 'Administration Confirmed'),('paid', 'Closed'), ('refunded', 'Refunded'),
         ('stop', 'Stoped'),('refuse', 'Refused')],
    ...
    def all_old_loans(self):
        for rec in self:
            balance = 0.0
            if rec.employee_id:
                loans_ids = rec.env['hr.loan.line'].search(
                    [('employee_id', '=', rec.employee_id.id),
                    ('paid', '=', False),('loan_id.state', '!=', 'stop')])
                for loan in loans_ids:
                    balance += loan.paid_amount
                rec.loan_amount_2 = balance

    @api.depends('employee_id','loan_amount')
    def compute_final_loan(self):
        for rec in self:
            rec.loan_amount_3 = rec.loan_amount + rec.loan_amount_2
    ...
    def _compute_amount(self):
        ...
        for loan in self:
            ...
            balance_amount = loan.loan_amount_3 - total_paid_amount
            loan.total_amount = loan.loan_amount_3
    ...
    def validate_month(self):
        for x in self:
            if x.no_month < 1:
                raise ValidationError(_("Loan period can't be less than 1 month"))
    ...
    def loan_validate(self):
        ...
        for loan in self:
            if not loan.employee_id.address_home_id:
                ValidationError(_('Employee must be have contact,please add it'))
            line_ids = []
            debit_sum = 0.0
            credit_sum = 0.0
            ...
            if loan.loan_amount_2 > 0.0:
                old_loan_line_ids = loan.env['hr.loan.line'].search(
                    [('employee_id', '=', loan.employee_id.id),
                    ('paid', '=', False),('loan_id','!=',loan.id)])
                old_loan_ids = old_loan_line_ids.mapped('loan_id')
                for old_loan in old_loan_ids:
                    old_loan.state = 'stop'
                    for old_loan_line in old_loan.loan_line_ids:
                        if not old_loan_line.paid:
                            old_loan_line.stop = True
        ...
        for loan in self:
            ...
            if loan.state == 'admin_approve':
                ...
                debit_line = (0, 0, {
                    'name': loan_name,
                    'partner_id': False,
                    'account_id': loan.payment_account.id,
                    'journal_id': journal_id,
                    'date': loan_pay_date,
                    'debit': amount > 0.0 and amount or 0.0,
                    'credit': amount < 0.0 and -amount or 0.0,
    ...
    def compute_loan_line(self):
        for loan in self:
            ...
            counter = 1
            amount_per_time = loan.loan_amount_3 / loan.no_month
                ...
                if i != loan.no_month:
                    ...
                    line_id = loan_line.create({
                        ...
                        'paid_amount': round(amount_per_time, 2) + round(
                            loan.loan_amount_3 - (round(amount_per_time, 2) * loan.no_month), 2),
                        'employee_id': loan.employee_id.id,
                        'loan_id': loan.id})
                counter += 1
    ...
    def action_paid_amount(self):
        for line in self:
            ...
            if not line.payroll_id:
                ...
                move = self.env['account.move'].create(move_dict)
                move.post()
                line.write({'paid': True, 'move_id': move.id, 'notes': 'Paid'})
        return True
        ...
        for x in self:
            if x.paid_amount:
                short_loan_ids = x.env['hr.monthlyloan'].search(
                    [('employee_id', '=', x.employee_id.id), ('state', '=', 'done'),('is_type','=','loan')])
                short_loan_amt = 0.00
                ...
                if x.paid_amount + short_loan_amt > x.loan_id.emp_salary:
                    raise ValidationError(_("Monthly Loan Cannot Exceed The Employee's Salary!"))
...
class HrMonthlyloan(models.Model):
    ...
    state = fields.Selection(
        [('draft', 'To Submit'),
         ('approve', 'Approved by Administration'), ('paid', 'Closed'),
         ('refuse', 'Refused')],
        'Status', default='draft', readonly=True)
    company_id = fields.Many2one('res.company', default=lambda self: self.env.company)
    total_deductions = fields.Float(string='Month Deductions',compute='compute_total_deductions',store=True)
    paid = fields.Boolean(string="Paid")
    payroll_id = fields.Many2one('hr.payslip', string="Payslip Ref.")
    ...
    incentive_amount = fields.Float(compute="_compute_incentive_amount",store=True)

    @api.onchange('employee_id')
    def onchange_emp(self):
        domain={}
        incentives = self.env['hr.incentive.line'].search([
            ('employee_id','=',self.employee_id.id)
            ])
        incentive_ids = incentives.mapped('incentive_id')
        if self.employee_id:
            domain = {
                'domain':{
                    'incentive_id':[('id','in',incentive_ids.ids)]}}
        return domain

    @api.onchange('employee_id')
    def change_incentive(self):
        for rec in self:
            if rec.incentive_id:
                rec.incentive_id = False

    @api.depends('incentive_id','employee_id')
    def _compute_incentive_amount(self):
        for rec in self:
            rec.incentive_amount = 0.0
            if rec.incentive_id:
                line = self.env['hr.incentive.line'].search([
                    ('employee_id','=',rec.employee_id.id),
                    ('incentive_id','=',rec.incentive_id.id)
                ],limit=1)
                rec.incentive_amount = line.amount

    @api.constrains('loan_amount')
    def loan_request_check(self):
        for loan in self:
            if loan.incentive_id:
                old_loans = self.env['hr.monthlyloan'].search([
                    ('employee_id','=',loan.employee_id.id),
                    ('incentive_id','=',loan.incentive_id.id),
                    ('date','>=',loan.date.replace(day=1)),
                    ('date','<=',loan.date.replace(day=calendar.monthrange(loan.date.year, loan.date.month)[1])),
                    ('id','!=',loan.id)
                ])
                old_loan_amount = 0.0
                for o_loan in old_loans:
                    old_loan_amount += o_loan.loan_amount
                print('++++++++++++++++++ old_loan_amount',old_loan_amount)
                if (old_loan_amount+loan.loan_amount) > loan.incentive_amount:
                    raise UserError(_('You cannot request this amount, You have exceeded (%s) from incentive amount!') % old_loan_amount)

                if loan.loan_amount > loan.incentive_amount:
                    raise UserError(_('You cannot request loan more than incentive amount!'))
            else:
                if loan.total_deductions >= loan.employee_id.contract_id.wage:
                    raise UserError(_('You cannot request loan when Month Deductions more than employee salary!'))

                if loan.loan_amount >= loan.employee_id.contract_id.wage:
                    raise UserError(_('You cannot request loan more than salary!'))
    ...
    def compute_total_deductions(self):
        for rec in self:
            if rec.employee_id:
                ...
                end_date = rec.date.replace(day=calendar.monthrange(rec.date.year, rec.date.month)[1])
                medical_amount = rec.employee_id.contract_id.medical_amount
                si = rec.employee_id.contract_id.wage * 0.08

                short_loans = self.env['hr.monthlyloan'].search([
                    ('employee_id','=',rec.employee_id.id),
                    ('date','>=',start_date),
                    ('date','<=',end_date),
                    ('paid','!=',True),
                ...
                for loan in short_loans:
                    sum_short_loans += loan.loan_amount

                long_loan = self.env['hr.loan.line'].search([
                    ('employee_id','=',rec.employee_id.id),
                ...
                sum_long_loans = 0.0
                for loan in long_loan:
                    sum_long_loans += loan.paid_amount

                rec.total_deductions = medical_amount + si + sum_short_loans + sum_long_loans

    # def compute_total_deductions(self):
    #     for rec in self:
    #         start_date = rec.date.replace(day=1)
    #         end_date = rec.date.replace(day=calendar.monthrange(rec.date.year, rec.date.month)[1])
    #         short_loan = self.env['hr.monthlyloan'].search([
    #             ('employee_id','=',rec.employee_id.id),
    #             ('date','>=',start_date),
    #             ('date','<=',end_date)
    #         ])

    #         long_loan = self.env['hr.monthlyloan'].search([
    #             ('employee_id','=',rec.employee_id.id),
    #             ('date','>=',start_date),
    #             ('date','<=',end_date)
    #         ])
    ...
    # def compute_total_deductions(self):
    #     for rec in self:
    #         payslip_month_ids = rec.env['hr.payslip'].search([])
    #         for month in payslip_month_ids:
    #             slip_id = month.id
    #             employee_id = month.employee_id.name
    #             slip_ids = rec.env['hr.payslip.line'].search([('slip_id', '=', slip_id),
    #                                                                  ('employee_id', '=', employee_id.id)])
    #             for slip_line in slip_ids:
    #                 category = slip_line.name
    ...
    def action_paid(self):
        ...
        for loan in self:
            ...
            if loan.state == 'approve':
                ...
                credit_line = (0, 0, {
                    'name': loan_name,
                    'partner_id': loan.employee_id.address_home_id,
                    'account_id': loan.employee_account.id,
                    'journal_id': journal_id,
                    'date': loan_pay_date,
                    'debit': amount < 0.0 and -amount or 0.0,
    ...
    def loan_validate(self):
        ...
        for monthh_loan in self:
            if not monthh_loan.employee_id.address_home_id:
                ValidationError(_('Employee must be have contact,please add it'))
            line_ids = []
            debit_sum = 0.0
            credit_sum = 0.0
            ...
            debit_line = (0, 0, {
                'name': loan_name,
                'partner_id': monthh_loan.employee_id.address_home_id.id,
    ...
    def unlink(self):
        for rec in self:
            if rec.state != 'draft':
                raise ValidationError(_("Warning! You cannot delete a Loan which is in %s state.") % (rec.state))
    ...
    def refund_loan(self):
        for loan in self:
            if loan.loan_id:
                ...
                total_amount = hr_loan_id.loan_amount_3
                ...
                if reaming_amount == 0:
                    if hr_loan_id.state == 'done':
                            ...
                            for loan_line in loan_line_ids:
                                if not hr_loan_id.employee_id.address_home_id:
                                    ValidationError(_('Employee must be have contact,please add it'))
                                paid = loan_line.paid
                                if not paid:
                                    loan_line.paid = True
                                    ...
                                    credit_line = (0, 0, {
                                        'name': ref_loan_name,
                                        'partner_id': hr_loan_id.employee_id.address_home_id.id,
